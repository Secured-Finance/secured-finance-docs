...
| **Active position**      | Future Value (FV)            | Valued on read; when its market matures, it's _treated as_ rolled without any write                               |
...
* **Auto-Roll** without lazy evaluation would mean updating _every position in a market_ at maturity — thousands of writes in one block, breaching gas limits. With GV accounting it's a single factor update.
...
No. The derivations are deterministic: given the same on-chain facts, a read always produces the same value, whether cleanup has run or not. Deferral changes _when_ storage is written, never _what_ the position is worth.

...
The Circuit Breaker limits how far prices can move **within a single block**, for both market and limit orders. Orders that would execute outside the allowed range are [blocked](../core-concepts/order-life-cycle.md) rather than filled. Because the limit binds per block, a flash-loan attacker cannot move the price dramatically inside one transaction — the core defense this mechanism provides.
...
Formally, for a requested price *P*:

```
Upper bound  = Min( Max(MA3 × 1.10, MA3 + 7.00), P )
Lower bound  = Max( Min(MA5 × 0.95, MA5 − 2.00), P )
```
...
* A very large market order may **partially fill** up to the boundary; the remainder is **Blocked → Killed**.

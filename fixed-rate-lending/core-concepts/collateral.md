...
Borrowing on the Fixed-Rate Lending Protocol requires **over-collateralization**: you deposit assets worth more than you borrow. This replaces credit checks — anyone can borrow, and lenders are protected because under-collateralized positions are [liquidated](liquidation/).
...
* **iFIL / wpFIL (liquid staking tokens)** — represent staked FIL that keeps earning staking rewards _while_ serving as collateral. For example, stake FIL on [GLIF](https://glif.io/) to receive iFIL, then deposit the iFIL as collateral — a dual-yield position. Both are accepted on Filecoin.
...
* If LTV reaches the liquidation threshold, the position can be [liquidated](liquidation/).

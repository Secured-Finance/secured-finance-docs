...
Execution involves **no per-trade discretionary decisions**. Strategy parameters are either fixed at deployment or adjustable only through disclosed governance functions restricted to the management role; every parameter change is an on-chain transaction, publicly visible and permanently auditable. The strategy contract's logic is **not upgradeable**, and share accounting is delegated to Yearn v3's audited `TokenizedStrategy` implementation. Full detail is documented on the [JPYC Lending Strategy](jpyc-lending-strategy.md#automated-execution-and-governance) page, which describes the same framework.
